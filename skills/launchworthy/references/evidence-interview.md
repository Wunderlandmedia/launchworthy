...
| Failed-payment retries and dunning (D2, payments) | Read the webhook first: access granted on success with no `invoice.payment_failed` branch is a code-visible finding on its own | "In your payment provider, open the retry and dunning settings (Stripe: Billing > Revenue recovery / Smart Retries). Paste whether automatic retries and failed-payment emails are on. Then say what happens to access when a renewal finally fails: kept, revoked, or nothing." Nothing configured means renewal failures are silent revenue loss; record dashboard-only answers as reported-unverified. |
| Refund and cancellation policy at checkout (D2, payments) | Grep the checkout/pricing/paywall route for a refund, cancellation, or terms link yourself | Ask only if the grep is inconclusive: "Is a refund or cancellation policy linked from the page where the customer pays? Paste the URL it links to." A policy that exists but is not reachable from the point of charge still counts as missing there. |
| Dispute readiness (D2, payments) | Read the webhooks first: no `charge.dispute.created` (or provider equivalent) handler is a code-visible gap | "In your provider's dispute settings, tell me how you are notified of a chargeback and, in one line, what evidence you would submit (proof of delivery, access logs, ToS acceptance). Paste the notification setting." No notification path means disputes are auto-lost on the deadline. |
| Stale authority data after revocation (D2) | Read the code first: authority data behind a TTL with no purge on the write path is a code-visible finding on its own, and the TTL value is the size of the window | "In your dashboard or database, suspend a test account (or cancel its subscription). Then, as that user, try the thing that should now be blocked. Paste the status code and say whether it still worked, and if it did, how long it kept working." Still working means the revocation window is real; the length of it is the severity. Nothing to test against means `reported by user, unverified`. |
| Account deletion actually erases the data (D2) | Read the code first: a delete path that removes only the auth user, or a foreign key to the user table with no `ON DELETE` behavior, is a code-visible finding on its own | "Create a throwaway account, add some data to it (a post, an upload, a profile field), then delete the account through the app. In your database, query for rows that still reference that user id and paste the count per table, not the contents. Anything left that is not a deliberately anonymized record means the deletion kept the PII." No self-serve deletion path at all stays a code-visible `[MEDIUM]`. |
| Privacy policy matches the data collected (D2) | Read the code first: reconcile the third parties the policy names against the analytics, payment, error-tracking, and email SDKs the code ships; a policy that names none while several are wired in is a code-visible contradiction | "Paste the data-sharing / third-parties section of your live privacy policy (not the whole document). I will compare it against the integrations I found in the code." A policy that omits an SDK the code loads, or claims no third-party sharing while a tracker is present, is a finding; legal sufficiency of the wording stays `MANUAL CHECK NEEDED`. |
